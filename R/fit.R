# kbpm_fit <- function( Data, env = NULL, pella = T, model = "Base") {
#
#   data <- Data$data
...
#   lowers <- c( 0.05, 0)
#
#   my_model <- function(x, r, K)   r * x * (1 - (x / K))
#   if (model == "Mult") my_model <- function( x, env, r, K, c)  r * x * (1 - (x / K)) * exp(env)
#   if (model == "Add") my_model <- function( x, env, r, K, c)  r * x * (1 - (x / K)) + env * x
#
#   if ( pella) {
#     start_p <- Data$start_p
#     params <- c( params, p = start_p)
#     uppers <- c( uppers, 3.5)
#     lowers <- c( lowers, 0.25)
#
#     my_model <- function( x, r, K, p){ (r / p) * x * (1 - (x / K)^p)}
#     if (model == "Mult") my_model <- function( x, env, r, K, p, c){ (r / p) * x * (1 - (x / K)^p) * exp(env)}
#     if (model == "Add") my_model <- function( x, env, r, K, p, c){ (r / p) * x * (1 - (x / K)^p) + env * x}
#   }
#
#   if ( n_env > 0) {
...
#   }
#
#   if( model == 'Base' & pella == T){ error_fun <- function( p, x, y){ sum((y-my_model(x, p["r"], p["K"], p["p"]))^2)}}
#   if( model == 'Base' & pella == F){ error_fun <- function( p, x, y){ sum((y-my_model(x, p["r"], p["K"]))^2)}}
#
#   if ( model != 'Base' & pella == T) error_fun <- function( p, x, y, env){
#       env <- if (n_env > 1) as.matrix(env) %*% p[4:(length(p))] else p['c'] * env
#       sum((y - my_model(x, env, p["r"], p["K"], p["p"], p[-(1:4)]))^2)}
#
#   if( model != 'Base' & pella == F) error_fun <- function( p, x, y, env){
...
#   if(model != 'Base') out <- optimx::optimr( params, error_fun, x = data$x, y = data$y, method = "L-BFGS-B", upper = uppers, lower = lowers, env = env)
#
#   if(model != 'Base'){ out <- out$par; attr( out,'status') <- NULL}
#
#   return(out)
#
# }
