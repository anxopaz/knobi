...
For simplicity, the output slot `$BRPs` provides the RPs estimates for a value of the centered environmental variable equal to 0 (*X=0*), see the following example.
...
The environmental fit information can be considered too in the retrospective analysis through the `env_results` argument, where the result of the `knobi_environmental` function has to be provided.

```{r,  fig.width=6, fig.height=4, fig.align = 'center'}
knobi_retro(hake_n_results, hake_n_environmental); hake_n_retros
```
...
```{r,  fig.width=6, fig.height=4, fig.align = 'center'}
projections <- knobi_proj( knobi_results=hake_n_results, Ct=Ct)
```
...
```{r,  fig.width=6, fig.height=4, fig.align = 'center'}
knobi_proj(hake_n_results, hake_n_environmental, Ct=Ct, env=env)
```
...
knobi_proj( hake_n_results, f=f, env_results=hake_n_environmental, env=env)
```
...
multiproj <- knobi_proj( hake_n_results, hake_n_multi, Ct=Ct[1:5,], env=env)
```
