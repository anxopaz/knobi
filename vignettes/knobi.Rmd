...
It should be noted that this vignette will not show all the possibilities of each of the functions contained in this package, but only those essential for the correct functioning of each function, as well as some alternatives. For more information, please consult the help for each of the functions of the package.
...
where *r* is the intrinsic population growth rate, *K* is the carrying capacity and *p* is the asymmetry parameter, which allows non-symmetrical production curves and, consequently, maximum production different from *K/2*.
...
2. `knobi_env`: analyzes the productivity changes in response to environmental fluctuations.
...
The data was downloaded using the `icesSAG` package, then it was uploaded and saved in the `hake_n` object.
...
In step (3) environmental covariables can be included as additive and multiplicative effects in the KBPM base formulation, i.e. Eq. (4).
...
```{r}
data <- list(
  env = data.frame( AMO=Env$AMO, Tmax=Env$TMax),
  years = Env$years)
```
...
From Eq. (5) and (6) we can derive the formulas that provide the reference points (RPs). It is important to take into account that in these models the RPs can depend on the value of the environmental covariate (details provided below for each model).
...
For simplicity, the output slot `$BRPs` provides the RPs estimates for a value of the centered environmental variable equal to 0 (*X=0*). The mathematical formulation of the RPs estimates for each KBPM model depending on the centered environmental variable are:
...
where *r*, *p*, *K* and *c* are the model parameter estimates of the Eq. (5) and (6) and *X* the centered environmental variable.
...
Furthermore, it is also possible to fit the environmental models considering several variables at the same time using `control$multicovar = TRUE`. This means that *cX<sub>t</sub>* is replaced by $\sum_{i=1}^{N} c_i X_{t,i}$ in Eq. (5) and (6), where index *N* represents the number of environmental variables.

Below you can see how we introduce the same data set as in previous examples but in the control we set `multicovar=TRUE` so that the two variables, “AMO” and “TMax”, are considered in the environmental fit. Note that “AMO” is 2 years lagged whereas “TMAX” is 3 years lagged respect the SP.
...
The estimated surplus production curves from the retrospective analysis are plotted. The plot is displayed in the plot window and also saved (note that `plot_out=T`) in the provided directory and file, which are the same as in the `knobi_fit` function.
...
In the current example, three scenarios are considered: (i) Constant maximum year temperature equal to 19 degrees (centigrade); (ii) temperature equal to 19 degrees and then constant raise of 0.5 degrees; and (iii) temperature equal to 19 degrees and then constant raise of 1 degrees.
...
Note that as you can see below, in this case, in addition to the plot with the results of the base KBPM model, different plot panels for each catch or fishing mortality scenario depending on the model and the environmental scenario; and, on the other hand, for each environmental scenario depending on the model and the catch or fishing mortality scenario are provided.

```{r,  fig.width=6, fig.height=4, fig.align = 'center'}
env_projections <- knobi_proj(hake_n_results, hake_n_environmental, c=C, env=env)
head(env_projections)
```
